...
import { getCurrentUser, consumeAiUse, releaseAiUse } from '@/lib/auth';
...
export const analyzeTender = async (tenderId: string): Promise<TenderAnalysis> => {
  ...
  const reserved = await consumeAiUse(username);
  if (!reserved) throw new Error('Wyczerpano limit analiz AI dla tego konta.');
